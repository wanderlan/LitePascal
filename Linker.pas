...
procedure Link(const ExeName: TString);
...
begin
  ...
  with Headers do begin
    ...
    with PEOptionalHeader do begin
      Magic         := $10B;                                                // PE32
      ...
      SizeOfImage   := Align(SizeOf(Headers), SECTALIGN) + Align(CodeSize, SECTALIGN) + Align(InitializedDataSize, SECTALIGN) +
        Align(UninitializedDataSize, SECTALIGN) + Align(ImportSize, SECTALIGN);
    ...
    with CodeSectionHeader do begin
      Name[0]          := '.';
      Name[1]          := 't';
      Name[2]          := 'e';
      Name[3]          := 'x';
    ...
    with DataSectionHeader do begin
      Name[0]          := '.';
      Name[1]          := 'd';
      Name[2]          := 'a';
      Name[3]          := 't';
    ...
    with BSSSectionHeader do begin
      Name[0]          := '.';
      Name[1]          := 'b';
      Name[2]          := 's';
      Name[3]          := 's';
    ...
    with ImportSectionHeader do begin
      Name[0]          := '.';
      Name[1]          := 'i';
      Name[2]          := 'd';
      Name[3]          := 'a';
...
function AddImportFunc(const ImportLibName, ImportFuncName: TString): Longint;
begin
  with ImportSectionData do begin
    Inc(NumImports);
    ...
    if ImportLibName <> LastImportLibName then begin
      Inc(NumImportLibs);
...
begin
  with ImportSectionData do begin
    ...
    ImportLibIndex     := 0;
    LookupIndex        := 0;
    for ImportIndex := 1 to NumImports do begin
      // Add new import library
      if (ImportLibIndex = 0) or (Import[ImportIndex].LibName <> LastImportLibName) then begin
        if ImportLibIndex <> 0 then
          Inc(LookupIndex);  // Add null entry before the first thunk of a new library
        Inc(ImportLibIndex);
...
begin
  with ImportSectionData do begin
    for i := 1 to NumImportLibs do
      with DirectoryTable[i] do begin
