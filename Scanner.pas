...
begin
  Result := False;
  if ScannerStackTop < SCANNERSTACKSIZE then begin
    Inc(ScannerStackTop);
...
begin
  Result := False;
  if ScannerStackTop > 0 then begin
...
begin
  if ScannerState.ch = #10 then
    Inc(ScannerState.Line);  // End of line found
  ch := #0;
  with ScannerState.Buffer do
...
begin
  with ScannerState do begin
    ...
    if Text = '$APPTYPE' then begin            // Console/GUI application type directive
      Text := '';
      ReadChar(ch);
      while (ch <> '}') and not EndOfUnit do begin
        if (ch = #0) or (ch > ' ') then
      ...
      if Text = '$UNITPATH' then begin       // Unit path directive
        Text := '';
        ReadChar(ch);
        while (ch <> '}') and not EndOfUnit do begin
          if (ch = #0) or (ch > ' ') then
...
begin
  with ScannerState do begin
    Num      := 0;
    NumFound := False;
    while ch in HexDigits do begin
...
begin
  with ScannerState do begin
    ...
    if (ch <> '.') and (ch <> 'E') then begin                                  // Integer number
      Token.Kind     := INTNUMBERTOK;
      Token.OrdValue := Num;
    end
    else begin
      ...
      if ch = '.' then begin
        ...
        if ch2 = '.' then begin                                                 // Integer number followed by '..' token
          Token.Kind     := INTNUMBERTOK;
          Token.OrdValue := Num;
          RangeFound     := True;
        end;
        if not EndOfUnit then
          Dec(Buffer.Pos);
      end; // if ch = '.'
      if not RangeFound then begin                                              // Fractional number
...
begin
  with ScannerState do begin
    ...
    while (ch in Spaces) or (ch = '{') or (ch = '/') do begin
      if ch = '{' then begin                                                      // Multi-line comment or directive
        ReadUppercaseChar(ch);
        if ch = '$' then
          ReadDirective
        else
    ...
    case ch of
      '0'..'9', '$': ReadNumber;
      '#': ReadCharCode;
      'A'..'Z', 'a'..'z', '_': ReadKeywordOrIdentifier;
      ...
      '.': begin
        ...
        case ch of
          ...
          else Error('Unexpected character or end of file');
        end; // case
        ReadChar(ch);
    end; // case
  end;
